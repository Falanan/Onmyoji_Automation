...
class finishInterfaceDetection(threading.Thread):
    def __init__(self, temp_path,click_list, *args, **kwargs):
        ...
        self.template_ds = self.gen_down_sample(img_t, 4) # generate down sampling for template

    def get_relative_size(self, img_shape):
        # img_shape = orig_img.shape
        temp_box_width = img_shape[1] / 672.0
        width = math.floor(temp_box_width * 432)
        temp_box_height = img_shape[0] / 465.0
        height = math.floor(temp_box_height * 146)
        return width, height
    ...
    def find_highest_match_box(self, bbox_list, img_shape):
        ...
        #         if i in x_dict and (x_dict[i] + x_dict[key] > 3):
        #             x_list.append(key)
        ...
        #     for i in range(key - 3, key + 3):
        #         # print("testNode1", i)
        #         if i in y_dict and (y_dict[i] + y_dict[key] > 3):
        #             # print("testNode2", key)
        #             y_list.append(key)
    ...
    def find_sign(self, orig_img):
        ...
        for template in self.template_ds: # match each template in the down sampling list
            # sq_temp_img, nlevels = make_square(template)
            # gpt = gen_gaussian_pyramid(sq_temp_img, nlevels) # for each down sampled template generate gaussian pyramid
        ...
        # print(bbox)
        # wid, heig = self.get_relative_size(orig_img.shape)
        # bbox.append(wid)
        # bbox.append(heig)
        return bbox
